...
**`[GDE-SPIN-130]` Mode C — lempipi as a plain Sendspin player.** The cheapest by far — `player@v1` only, no source or server role — and the only one where **the Director is bypassed entirely**: some other controller picks what plays, and lempipi is a synced speaker like any $10 ESP32. This has a real, narrow use: a housemate casting from Music Assistant's own library to the room lempipi occupies without displacing whatever lempipi itself is doing when idle. It earns no line in `[SPEC009]`'s own story and should be judged only as "the appliance can also be an ordinary Sendspin speaker when asked," never as a Director feature.
...
**`[GDE-SPIN-160]` The mapping problem GUIDE007 found for MPD does not recur here — a different one takes its place.** MPD needed a passage-to-server-song identity mapping `[GDE-BAK-025]`; Sendspin never asks Lempi to name a track to a peer at all, since audio travels as raw frames, not by reference. What it does ask for for Mode A/B is a **pairing UI** lempipi may not have the input hardware for — the PIN flows assume a display or a button `[GDE-SPIN-030]`; the Pairing-PSK flow avoids that but needs an out-of-band provisioning step (a QR code, a config file) instead.
