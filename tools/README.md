...
Everything here is Vipunen: the library-building side of Lempi, Python, AGPL-3.0-or-later — see [GUIDE002 §2](../docs/GUIDE002-rearchitecture-plan.md#2-architectural-decisions) (`[GDE-ARC-010]`) for why it's a separate entity from `player/`. 134 files in one flat directory with no subpackages, 128 of them `.py`; this index exists so "where is the code that does X" has an answer without moving anything, per `[GDE-CHT-040]`'s scope discipline — restructuring into subpackages was considered and set aside precisely because the flat layout is what every doc's CLI example, every path Vipunen's own job runner builds, and every `test_*.py`'s import already assume.

Each script is run directly — `python tools/<script>.py --help` for its own arguments — and most carry a runnable example in their own module docstring. `test_<name>.py` beside `<name>.py` is that module's own tests, run the same way (`python` on it, from `tools/`); a handful of `test_console_*.py` files test slices of `console.py` instead of a same-named module. **`pytest` is not the runner here and reports 59 errors if you try it** — every one of them `fixture 'tmp' not found`, because these files pass their own temporary directory in from a `__main__` block rather than taking a pytest fixture. The tests are fine; the runner is wrong. Fifty-two of the 53 pass when run directly with no arguments; the fifty-third, `test_split_parity.py`, is a harness rather than a self-contained test — it takes `--pair DIR --whole lempi.db` and runs the read-only tools against both database shapes to compare their answers (`HOWTO.md` §3b). Run bare it exits 2 on its own argument parser, which is not a failure of anything it tests.

---

## Console & orchestration

The Vipunen console — a local web UI over the library, and the job runner it drives. Stage 2/3 of [IMPL003](../docs/IMPL003-vipunen-console-build.md).
...
| `lempi_control.py` | Everything that talks to the co-resident player's own process or HTTP API: liveness, capability and staleness checks, starting it, signaling a flag write `[SPEC-SUI-170]`, `[SPEC-SUI-227]`. |
...
| `migrate_mulib.py` | Migrate MuLibPlay's database into the Lempi schema `[GDE-PHS-010]`. |
...
| script | what |
| :--- | :--- |
| `choose_release.py` | Which release is a file actually from? Vipunen S3, selection half. |
| `fetch_releases.py` | Fill `releases` and `release_recordings` `[SPEC-SA-030]`. Vipunen S3, release half. |
| `fetch_chosen_tracks.py` | The track listing of each chosen release. Vipunen S3, third pass. |
...
| `import_lyrics.py` | Bring MuLibPlay's lyrics into Lempi `[SPEC-LYR-030]`. |
...
| `check_forbidden_names.py` | Fails if a retired project name appears outside the one file allowed to record it `[GOV-FBN-010..020]`. |
