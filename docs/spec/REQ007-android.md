...
**`[REQ-AND-315]` A tags-only passage also rotates by its artist, keyed on the normalised artist tag** — accepted by the maintainer 2026-09-26 as the fallback where no proper flavor and identity data exists. *Built 2026-09-26* in `lempi-core`'s Director, keyed `tag:<normalised name>`: on the desktop's catalogue it reaches 89 radio passages across 9 tag artists that had no artist rotation at all. Without it, a newly found album rotates track by track and nothing stops several of its tracks playing close together — the artist tier exists for exactly that and is keyed by MBID, which a found file lacks. The key would be kept distinct from MBIDs, so a tag name can never collide with or block an identified artist.
...
**`[REQ-AND-530]` The device spike passes before this document leaves draft** `[GDE-APP-020]`: a passage audible for 30 minutes screen-off with zero underruns, a measured drain, the skin controlling it in a WebView. *Run 2026-09-26, [LOG013](../LOG013-the-android-spike.md):* 37.8 minutes audible screen-off with no underrun during it; a drain of at most 3.2–4.8% per hour; and the skin in control — pause, play, skip, volume and seek confirmed by the maintainer by hand `[LOG-SPK-920]`. **Passed.** Each start underruns briefly `[LOG-SPK-040]`. §7's open items were settled the same day, so what keeps this draft is only the full-library memory measurement `[REQ-AND-910]`.
...
5. **`[REQ-AND-940]` The Director's handling of a passage without flavor** `[REQ-AND-310]`. *Answered from the code 2026-09-25* — see `[REQ-AND-310]`; it raised `[REQ-AND-315]`, accepted 2026-09-26.
...
7. **`[REQ-AND-960]` How Vipunen recognises a file it holds, from the phone's description** `[REQ-AND-288]`. The phone has only the byte hash `[REQ-AND-270]`, and Vipunen keys its files by `audio_md5`, recording no byte hash for them. A byte-hash match proves a duplicate; tags and duration can only suggest one. *Decided 2026-09-26: Vipunen records the byte hash of every file it holds* — at induction, and once over the existing collection — so the negotiation `[REQ-AND-288]` answers exactly, and tags with duration flag the near matches its review shows `[REQ-AND-289]`. *Built 2026-09-26* as `files.sha256` `[SPEC-SC-039]`: set at induction, kept by a bundle import, and filled for existing rows by `tools/add_byte_hashes.py` — not yet run, since which copy is the primary awaits the maintainer (`data/README.md`).
