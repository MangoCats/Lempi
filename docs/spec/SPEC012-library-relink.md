...
Applies `[SPEC-DF-030]`'s identity keys to deployment. Companion: [SPEC006](SPEC006-data-flow-and-portability.md); the hash it binds by is [SPEC045](SPEC045-the-identity-hash.md).
...
[IMPL003](../IMPL003-vipunen-console-build.md).
...
This is MuLibPlay's `scanFile` `[GDE-BMK-050]`, which hashed a **file**. Lempi
...
*Designed, not yet built.* `player/core/src/relink.rs` and `player/src/bin/relink.rs`
...
**Which tool computes the hash, and why that is not a free choice**, moved
to [SPEC045](SPEC045-the-identity-hash.md) on 2026-09-24:
`[SPEC-RLK-080]` through `[SPEC-RLK-088]`, and the deferred change of hash
authority `[SPEC-RLK-150]`. Relink uses the hash; that document is about the
hash.
...
easily forgotten" `[GDE-BMK-050]`. Lempi takes the relocation without the
